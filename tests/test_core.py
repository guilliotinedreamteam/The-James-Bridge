...
from tests.conftest import ARPABET
from tests.conftest import ecog as mk_ecog
from tests.conftest import labels as mk_labels
...
class TestConfigDefaults:
    def test_timesteps(self):
        ...
        assert C.NUM_TIMESTEPS == 100

    def test_features(self):
        from neurobridge.config import Config as C

        assert C.NUM_FEATURES == 128

    def test_phonemes(self):
        from neurobridge.config import Config as C

        assert C.NUM_PHONEMES == 41

    def test_lstm(self):
        from neurobridge.config import Config as C

        assert C.LSTM_UNITS == 256

    def test_dense(self):
        from neurobridge.config import Config as C

        assert C.DENSE_UNITS == 128
    ...
    def test_sr(self):
        ...
        assert C.AUDIO_SAMPLING_RATE == 16000

    def test_dur(self):
        from neurobridge.config import Config as C

        assert C.PHONEME_DURATION_SEC == 0.1
...
class TestConfigPaths:
    def test_model_ext(self):
        ...
        assert str(C.model_save_path()).endswith(".keras")

    def test_rt_ext(self):
        from neurobridge.config import Config as C

        assert str(C.realtime_model_save_path()).endswith(".keras")

    def test_dir_created(self, tmp_path):
        from neurobridge.config import Config as C

        orig = C.MODEL_DIR
        C.MODEL_DIR = tmp_path / "m"
        try:
            C.model_save_path()
            assert C.MODEL_DIR.exists()
        finally:
            C.MODEL_DIR = orig
...
class TestConfigSummary:
    ...
    def test_fields(self):
        ...
        s = C.summary()
        assert all(k in s for k in ["Timesteps", "Features", "Phonemes", "LSTM"])
...
class TestPhonemeMap:
    def test_len(self):
        ...
        assert len(C.PHONEME_MAP) == C.NUM_PHONEMES
...
class TestEnvOverrides:
    def _reload(self):
        ...
        import neurobridge.config as cfg
    ...
    def test_ts(self):
        with patch.dict(os.environ, {"NB_NUM_TIMESTEPS": "200"}):
            try:
                assert self._reload().NUM_TIMESTEPS == 200
            finally:
                self._reload()
    ...
    def test_feat(self):
        with patch.dict(os.environ, {"NB_NUM_FEATURES": "64"}):
            try:
                assert self._reload().NUM_FEATURES == 64
            finally:
                self._reload()
    ...
    def test_port(self):
        with patch.dict(os.environ, {"NB_API_PORT": "8080"}):
            try:
                assert self._reload().API_PORT == 8080
            finally:
                self._reload()
    ...
    def test_lr(self):
        with patch.dict(os.environ, {"NB_LEARNING_RATE": "0.01"}):
            try:
                assert self._reload().LEARNING_RATE == 0.01
            finally:
                self._reload()
...
class TestConfigConsistency:
    ...
    def test_ts_pos(self):
        ...
        assert C.NUM_TIMESTEPS > 0

    def test_feat_pos(self):
        from neurobridge.config import Config as C

        assert C.NUM_FEATURES > 0

    def test_ph_pos(self):
        from neurobridge.config import Config as C

        assert C.NUM_PHONEMES > 0

    def test_lstm_pos(self):
        from neurobridge.config import Config as C

        assert C.LSTM_UNITS > 0

    def test_host_str(self):
        from neurobridge.config import Config as C

        assert isinstance(C.API_HOST, str)

    def test_sr_range(self):
        from neurobridge.config import Config as C

        assert 8000 <= C.AUDIO_SAMPLING_RATE <= 48000

    def test_dur_pos(self):
        from neurobridge.config import Config as C

        assert 0 < C.PHONEME_DURATION_SEC < 1.0
...
class TestData:
    def test_shape(self):
        from neurobridge.data import generate_mock_ecog_data as G

        assert G(5, timesteps=10, features=16).shape == (5, 10, 16)

    def test_dtype(self):
        from neurobridge.data import generate_mock_ecog_data as G

        assert G(3, timesteps=5, features=8).dtype == np.float32

    def test_bounds(self):
        from neurobridge.data import generate_mock_ecog_data as G

        d = G(10, timesteps=5, features=8)
        assert d.min() >= 0 and d.max() <= 1

    def test_finite(self):
        from neurobridge.data import generate_mock_ecog_data as G

        assert np.all(np.isfinite(G(10, timesteps=50, features=32)))

    def test_varies(self):
        from neurobridge.data import generate_mock_ecog_data as G

        d = G(5, timesteps=10, features=8)
        assert not np.array_equal(d[0], d[1])

    def test_lbl_shape(self):
        from neurobridge.data import generate_mock_phoneme_labels as G

        assert G(5, timesteps=10, num_classes=3).shape == (5, 10, 3)

    def test_lbl_onehot(self):
        from neurobridge.data import generate_mock_phoneme_labels as G

        np.testing.assert_allclose(G(5, timesteps=10, num_classes=3).sum(axis=-1), 1.0)

    def test_lbl_vals(self):
        from neurobridge.data import generate_mock_phoneme_labels as G

        assert set(np.unique(G(10, timesteps=20, num_classes=5))).issubset({0.0, 1.0})

    def test_gen_batch(self):
        from neurobridge.data import create_data_generator as G

        e, l = next(G(batch_size=4))
        assert e.shape[0] == 4 and l.shape[0] == 4

    def test_gen_match(self):
        from neurobridge.data import create_data_generator as G

        e, l = next(G(batch_size=4))
        assert e.shape[0] == l.shape[0] and e.shape[1] == l.shape[1]

    def test_gen_dtype(self):
        from neurobridge.data import create_data_generator as G

        e, l = next(G(batch_size=3))
        assert e.dtype == np.float32 and l.dtype == np.float32

    def test_real_loader_missing(self):
        from neurobridge.data import load_real_ecog_data as L

        assert L("/nonexistent/x.mat") is None
...
class TestEvaluate:
    def _m(self):
        ...
        from neurobridge.model import compile_model as C
    ...
    def test_all_keys(self, ecog_4x5x8, labels_4x5x3):
        ...
        assert all(
            ...
            for k in [
                "phoneme_accuracy",
                "best_sample_accuracy",
                "worst_sample_accuracy",
                "num_test_samples",
                "num_timesteps",
            ]
        )
    ...
    def test_count(self, ecog_4x5x8, labels_4x5x3):
        ...
        assert (
            E(model=self._m(), test_ecog=ecog_4x5x8, test_labels=labels_4x5x3)[
                "num_test_samples"
            ]
            == 4
        )
    ...
    def test_single_sample(self):
        ...
        e = mk_ecog(8, 5, seed=42).reshape(1, 5, 8)
        l = mk_labels(5, 3, seed=42).reshape(1, 5, 3)
    ...
    def test_no_labels_raises(self):
        ...
        with pytest.raises(ValueError):
            E(
                model=self._m(),
                test_ecog=mk_ecog(8, 5).reshape(1, 5, 8),
                test_labels=None,
            )
...
class TestDecode:
    ...
    def test_load_missing(self):
        ...
        from neurobridge.evaluate import load_model as L

        with pytest.raises(FileNotFoundError):
            L(Path("/nonexistent/m.keras"))
...
class TestInference:
    def _d(self, f=16, c=5):
        ...
        d = RealtimeDecoder()
        d.model = build_realtime_decoder(features=f, num_classes=c, lstm_units=16)
        d._features = f
        d._num_phonemes = c
        return d
    ...
    def test_init_none(self):
        ...
        assert RealtimeDecoder().model is None

    def test_no_model_raises(self):
        from neurobridge.inference import RealtimeDecoder

        with pytest.raises(RuntimeError):
            RealtimeDecoder().predict(np.zeros(128, dtype=np.float32))

    def test_wrong_shape(self):
        with pytest.raises(ValueError):
            self._d().predict(np.zeros((2, 16), dtype=np.float32))

    def test_wrong_feat(self):
        with pytest.raises(ValueError):
            self._d().predict(np.zeros(32, dtype=np.float32))

    def test_shape(self, frame16):
        assert self._d().predict(frame16).shape == (5,)

    def test_sums(self, frame16):
        np.testing.assert_allclose(self._d().predict(frame16).sum(), 1.0, atol=1e-5)

    def test_label(self, frame16):
        l = self._d().predict_label(frame16, phoneme_map=ARPABET[:5])
        assert l in ARPABET[:5]

    def test_topk_count(self, frame16):
        assert len(self._d().predict_top_k(frame16, k=3, phoneme_map=ARPABET[:5])) == 3

    def test_topk_sorted(self, frame16):
        p = [
            x[1] for x in self._d().predict_top_k(frame16, k=5, phoneme_map=ARPABET[:5])
        ]
        assert p == sorted(p, reverse=True)

    def test_topk_unique(self, frame16):
        ls = [
            x[0] for x in self._d().predict_top_k(frame16, k=5, phoneme_map=ARPABET[:5])
        ]
        assert len(ls) == len(set(ls))

    def test_standalone_shape(self, frame8):
        from neurobridge.inference import predict_realtime_phoneme as P
    ...
    def test_standalone_sums(self, frame8):
        ...
        np.testing.assert_allclose(
            P(frame8, B(features=8, num_classes=3, lstm_units=16)).sum(), 1.0, atol=1e-5
        )
    ...
    def test_zeros(self):
        d = self._d()
        p = d.predict(np.zeros(16, dtype=np.float32))
        np.testing.assert_allclose(p.sum(), 1.0, atol=1e-5)

    def test_ones(self):
        d = self._d()
        p = d.predict(np.ones(16, dtype=np.float32))
        np.testing.assert_allclose(p.sum(), 1.0, atol=1e-5)

    def test_negative(self):
        d = self._d()
        p = d.predict(np.full(16, -0.5, dtype=np.float32))
        assert not np.any(np.isnan(p))

    def test_large(self):
        assert not np.any(
            np.isnan(self._d().predict(np.full(16, 500, dtype=np.float32)))
        )
    ...
    def test_topk_over_classes(self):
        ...
        assert (
            len(
                d.predict_top_k(
                    mk_ecog(8, 1, seed=42).flatten(), k=10, phoneme_map=ARPABET[:3]
                )
            )
            == 3
        )
    ...
    def test_load_missing(self, tmp_path):
        ...
        Config.MODEL_DIR = tmp_path / "empty"
    ...
    def test_float64_cast(self):
        ...
        assert P(
            np.linspace(0, 1, 8).astype(np.float64),
            B(features=8, num_classes=3, lstm_units=16),
        ).shape == (3,)
...
class TestSynth:
    def test_array(self):
        from neurobridge.synthesizer import \
            synthesize_speech_from_phonemes as S
    ...
    def test_len(self):
        ...
        assert (
            len(S(H, sampling_rate=8000, phoneme_duration_sec=0.1, phoneme_map=ARPABET))
            == 3200
        )
    ...
    def test_single(self):
        ...
        assert (
            len(
                S(
                    np.array([39], dtype=np.int32),
                    sampling_rate=16000,
                    phoneme_duration_sec=0.05,
                    phoneme_map=ARPABET,
                )
            )
            == 800
...
class TestPipeline:
    def test_train_1_epoch(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            p = Path(d) / "m.keras"
            h = train_model(epochs=1, batch_size=4, train_samples=8, save_path=p)
    ...
    def test_rt_e2e(self):
        ...
        m = B(features=16, num_classes=5, lstm_units=16)
        C(m)
    ...
    def test_roundtrip(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            p = str(Path(d) / "rt.keras")
            m.save(p)
            np.testing.assert_allclose(
                m.predict(x, verbose=0),
                tf.keras.models.load_model(p).predict(x, verbose=0),
                atol=1e-5,
            )
    ...
    def test_decode_then_synth(self):
        ...
        m = B(timesteps=5, features=8, num_classes=3, lstm_units=16)
        ids = np.argmax(
            m.predict(mk_ecog(8, 5, seed=42).reshape(1, 5, 8), verbose=0)[0], axis=-1
        )
        a = S(
            ids, sampling_rate=8000, phoneme_duration_sec=0.1, phoneme_map=ARPABET[:3]
        )
    ...
    def test_train_eval_e2e(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            p = Path(d) / "m.keras"
            train_model(epochs=1, batch_size=4, train_samples=8, save_path=p)
            m = tf.keras.models.load_model(str(p))
            e = np.stack(
                [
                    mk_ecog(Config.NUM_FEATURES, Config.NUM_TIMESTEPS, seed=i)
                    for i in range(4)
                ]
            )
            l = np.stack(
                [
                    mk_labels(Config.NUM_TIMESTEPS, Config.NUM_PHONEMES, seed=i + 500)
                    for i in range(4)
                ]
            )
            assert (
                0
                <= evaluate_model(model=m, test_ecog=e, test_labels=l)[
                    "phoneme_accuracy"
                ]
                <= 1
            )
