...
class TestWeights:
    ...
    def test_rt_weights_finite(self):
        ...
        assert all(
            ...
            for w in B(features=8, num_classes=3, lstm_units=16).get_weights()
        )
...
class TestDataEdge:
    def test_1_sample(self):
        ...
        assert G(1, timesteps=5, features=8).shape == (1, 5, 8)

    def test_1_timestep(self):
        from neurobridge.data import generate_mock_ecog_data as G

        assert G(3, timesteps=1, features=8).shape == (3, 1, 8)

    def test_1_feature(self):
        from neurobridge.data import generate_mock_ecog_data as G

        assert G(3, timesteps=5, features=1).shape == (3, 5, 1)

    def test_large_dims(self):
        from neurobridge.data import generate_mock_ecog_data as G

        assert G(2, timesteps=500, features=256).shape == (2, 500, 256)

    def test_binary_labels(self):
        from neurobridge.data import generate_mock_phoneme_labels as G

        l = G(3, timesteps=5, num_classes=2)
        assert l.shape == (3, 5, 2)
        np.testing.assert_allclose(l.sum(axis=-1), 1.0)

    def test_100_classes(self):
        from neurobridge.data import generate_mock_phoneme_labels as G

        assert G(2, timesteps=3, num_classes=100).shape == (2, 3, 100)
...
class TestAPIExtra:
    ...
    def test_process_signal_with_nan_handled(self, c):
        ...
        r = c.post(
            "/api/process",
            data=json.dumps({"signal": [float("nan")] * 128}),
            content_type="application/json",
        )
...
class TestSynthExtra:
    def test_long_sequence(self):
        ...
        a = S(ids, sampling_rate=8000, phoneme_duration_sec=0.1, phoneme_map=ARPABET)
        assert len(a) == 16000
    ...
    def test_different_sampling_rate(self):
        ...
        a1 = S(ids, sampling_rate=8000, phoneme_duration_sec=0.1, phoneme_map=ARPABET)
        a2 = S(ids, sampling_rate=16000, phoneme_duration_sec=0.1, phoneme_map=ARPABET)
...
class TestLabelDiversity:
    def test_labels_vary_across_inputs(self):
        ...
        labels = {
            d.predict_label(
                mk_ecog(16, 1, seed=i * 100).flatten(), phoneme_map=ARPABET[:5]
            )
            for i in range(20)
        }
        assert len(labels) >= 1
...
class TestEvalExtra:
    def test_alternating_silence_decode(self):
        ...
        assert D(np.array([39, 0, 1, 39, 0]), phoneme_map=ARPABET) == [
            "SIL",
            "AA",
            "AE",
            "SIL",
