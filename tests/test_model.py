...
class TestOfflineDecoder:
    ...
    def test_output_shape(self, ecog_2x5x8):
        ...
        assert B(timesteps=5, features=8, num_classes=3).predict(
            ecog_2x5x8, verbose=0
        ).shape == (2, 5, 3)
    ...
    def test_softmax(self, ecog_1x5x8):
        ...
        np.testing.assert_allclose(
            B(timesteps=5, features=8, num_classes=3)
            .predict(ecog_1x5x8, verbose=0)
            .sum(axis=-1),
            1.0,
            atol=1e-5,
        )
    ...
    def test_compiles(self):
        ...
        m = B(timesteps=5, features=8, num_classes=3)
        C(m)
...
class TestRealtimeDecoder:
    ...
    def test_output_shape(self, rt_frame16):
        ...
        assert B(features=16, num_classes=5).predict(rt_frame16, verbose=0).shape == (
            1,
            5,
        )
    ...
    def test_softmax(self, rt_frame16):
        ...
        np.testing.assert_allclose(
            B(features=16, num_classes=5).predict(rt_frame16, verbose=0).sum(),
            1.0,
            atol=1e-5,
        )
...
class TestLayers:
    ...
    def test_rt_no_bidir(self):
        ...
        assert not any(
            "bidirectional" in l.name.lower()
            for l in B(features=8, num_classes=3).layers
        )
...
class TestConfigs:
    def test_small(self):
        ...
        assert (
            B(
                timesteps=2, features=4, num_classes=2, lstm_units=16, dense_units=8
            ).count_params()
            > 0
        )
    ...
    def test_100_classes(self):
        ...
        x = np.ones((1, 3, 4), dtype=np.float32) * 0.5
        assert B(
            timesteps=3, features=4, num_classes=100, lstm_units=16, dense_units=8
        ).predict(x, verbose=0).shape == (1, 3, 100)

    def test_1_timestep(self):
        from neurobridge.model import build_neurobridge_decoder as B

        x = np.ones((1, 1, 4), dtype=np.float32) * 0.5
        assert B(
            timesteps=1, features=4, num_classes=3, lstm_units=16, dense_units=8
        ).predict(x, verbose=0).shape == (1, 1, 3)

    def test_1_feature(self):
        from neurobridge.model import build_neurobridge_decoder as B

        x = np.ones((1, 3, 1), dtype=np.float32) * 0.5
        assert B(
            timesteps=3, features=1, num_classes=3, lstm_units=16, dense_units=8
        ).predict(x, verbose=0).shape == (1, 3, 3)

    def test_more_units_more_params(self):
        from neurobridge.model import build_neurobridge_decoder as B

        assert (
            B(timesteps=3, features=4, num_classes=3, lstm_units=64).count_params()
            > B(timesteps=3, features=4, num_classes=3, lstm_units=32).count_params()
        )
...
class TestDeterminism:
    ...
    def test_diff_in_diff_out(self, ecog_2x5x8):
        ...
        o = B(timesteps=5, features=8, num_classes=3, lstm_units=16).predict(
            ecog_2x5x8, verbose=0
        )
    ...
    def test_batch_vs_single(self, ecog_2x5x8):
        ...
        np.testing.assert_allclose(
            m.predict(ecog_2x5x8, verbose=0)[0],
            m.predict(ecog_2x5x8[0:1], verbose=0)[0],
            atol=1e-5,
        )
...
class TestSerialization:
    ...
    def test_save_load_realtime(self):
        ...
        x = np.ones((1, 1, 8), dtype=np.float32) * 0.5
        with tempfile.TemporaryDirectory() as d:
            p = os.path.join(d, "rt.keras")
            m.save(p)
            np.testing.assert_allclose(
                m.predict(x, verbose=0),
                tf.keras.models.load_model(p).predict(x, verbose=0),
...
class TestEdgeCases:
    ...
    def test_large_vals(self):
        assert not np.any(
            np.isnan(
                self._m().predict(
                    np.ones((1, 3, 4), dtype=np.float32) * 1000, verbose=0
                )
            )
        )
    ...
    def test_big_batch(self):
        assert self._m().predict(
            np.ones((64, 3, 4), dtype=np.float32) * 0.5, verbose=0
        ).shape == (64, 3, 3)
