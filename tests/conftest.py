...
def ecog(ch, ts, seed=42):
    ...
    for c in range(ch):
        ...
        sig[:, c] = (20*np.sin(2*np.pi*(8+c*.2)*t+ph[0])
                     + 10*np.sin(2*np.pi*(18+c*.3)*t+ph[1])
                     + 5*np.sin(2*np.pi*(80+c*1.5)*t+ph[2])
                     + rng.randn(ts)*2)
    mn, mx = sig.min(), sig.max()
    return ((sig - mn) / (mx - mn) if mx > mn else np.zeros_like(sig)).astype(np.float32)
