...
def get_decoder_and_trainer():
    try:
        ...
        logger.error(
            "Phase 3/4 dependencies missing. Ensure TensorFlow is installed via 'pip install tensorflow'."
        )
        sys.exit(1)
...
def _process_file(file_path: str) -> np.ndarray:
    ...
    downsampled = processor.downsample_signals(
        data_array, current_freq=int(raw_data.info["sfreq"])
    )
...
def handle_ingest(args):
    ...
    try:
        ...
        logger.info(
            f"Ingestion & Processing successful. Final sequence shape: {shaped.shape}"
        )
        ...
        if getattr(args, "decode", False):
            ...
            logger.info(
                f"Phase 3 Decoding successful. Output prediction tensor shape: {predictions.shape}"
            )
...
def handle_train(args):
    ...
    if getattr(args, "labels", None):
        ...
        y_train = aligner.align_to_tensor(
            df, total_original_samples=data_array.shape[1]
        )
        logger.info("Successfully aligned clinical labels to ground-truth tensor.")
    else:
        logger.warning(
            "No clinical phoneme labels provided (--labels). Generating synthetic targets for training validation."
        )
...
def handle_tune(args):
    ...
    if not args.labels:
        raise ValueError(
            "Legitimate clinical labels (--labels) are required for tuning."
        )
    ...
    tuner.execute_grid_search(
        lstm_options=[128, 256], dropout_options=[0.2, 0.4], epochs=args.epochs
    )
...
def handle_serve(args):
    ...
    try:
        ...
        logger.info(
            f"Server starting on {args.host}:{args.port}. Waiting for ECoG streams..."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Neurobridge V1 - Medical BCI Pipeline"
    )
    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    # Command: info
    parser_info = subparsers.add_parser(
        "info", help="Display system capabilities and configuration"
    )
    parser_info.set_defaults(func=handle_info)

    # Command: ingest
    parser_ingest = subparsers.add_parser(
        "ingest", help="Ingest a clinical ECoG dataset (.edf or .bdf)"
    )
    parser_ingest.add_argument(
        "--file",
        type=str,
        required=True,
        help="Path to the .edf or .bdf medical dataset",
    )
    parser_ingest.add_argument(
        "--decode",
        action="store_true",
        help="Run Phase 3 Neural Decoding (dry-run) after processing",
    )
    parser_ingest.set_defaults(func=handle_ingest)

    # Command: train
    parser_train = subparsers.add_parser(
        "train", help="Train the offline Bidirectional LSTM model"
    )
    parser_train.add_argument(
        "--file",
        type=str,
        required=True,
        help="Path to the .edf or .bdf medical training dataset",
    )
    parser_train.add_argument(
        "--labels",
        type=str,
        default=None,
        help="Path to the BIDS .tsv events file for clinical target alignment",
    )
    parser_train.add_argument(
    ...
    parser_tune = subparsers.add_parser(
        "tune", help="Run hyperparameter tuning on clinical data"
    )
    parser_tune.add_argument(
        "--file", type=str, required=True, help="Path to the dataset file"
    )
    parser_tune.add_argument(
        "--labels", type=str, required=True, help="Path to the BIDS .tsv events file"
    )
    parser_tune.add_argument(
        "--epochs", type=int, default=2, help="Epochs per tuning run"
    )
    parser_tune.set_defaults(func=handle_tune)

    # Command: serve
    parser_serve = subparsers.add_parser(
        "serve", help="Spin up the real-time inference server"
    )
    parser_serve.add_argument(
        "--host", type=str, default="0.0.0.0", help="Host binding"
    )
    parser_serve.add_argument("--port", type=int, default=8000, help="Port binding")
    parser_serve.add_argument(
        "--actuation",
        type=str,
        choices=["simulated", "tcp"],
        default="simulated",
        help="Phase 7 Actuation Mode",
    )
