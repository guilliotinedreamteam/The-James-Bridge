...
class SignalProcessor:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized SignalProcessor (Target Freq: {self.target_freq}Hz, Timesteps: {self.target_timesteps})"
        )
    ...
    def extract_high_gamma(self, data: np.ndarray, sfreq: int) -> np.ndarray:
        ...
        if sfreq <= 150 * 2:
            logger.warning(
                f"Sampling frequency {sfreq}Hz is too low for 150Hz High-Gamma extraction. Skipping."
            )
            return data
    ...
    def downsample_signals(
        self, data: np.ndarray, current_freq: Optional[int] = None
    ) -> np.ndarray:
        """
        ...
        logger.info(
            f"Downsampling signals from {current_freq} Hz to {self.target_freq} Hz"
        )
    ...
    def z_score_normalize(self, data: np.ndarray) -> np.ndarray:
        ...
        """
        logger.info(
            "Applying Z-score normalization across the time axis (per channel)."
        )
    ...
    def shape_sequences(self, data: np.ndarray) -> np.ndarray:
        ...
        """
        logger.info(
            f"Shaping continuous sequence into blocks of {self.target_timesteps} timesteps."
        )
        ...
        if num_blocks == 0:
            raise ValueError(
                f"Not enough data to create even one sequence of {self.target_timesteps} timesteps."
            )
        ...
        reshaped_data = truncated_data.reshape(
            (num_blocks, self.target_timesteps, channels)
        )
    ...
    def align_targets(
        self, phoneme_ids: np.ndarray, num_classes: int = 41
    ) -> np.ndarray:
        """
        Maps sparse phoneme IDs to a one-hot encoded matrix for Categorical Crossentropy.
        """
        logger.info(
            f"Aligning sparse phoneme targets to {num_classes}-class one-hot vectors."
        )
