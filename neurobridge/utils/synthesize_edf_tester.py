...
def create_simulated_clinical_edf(filepath: str, n_channels: int = 128, sfreq: int = 1000, duration_seconds: int = 10):
    """
    ...
    data = np.random.randn(n_channels, duration_seconds * sfreq) * 1e-6 # scale to microvolts

    # Create info structure required by MNE
    ch_names = [f'ECoG_{i+1}' for i in range(n_channels)]
    ch_types = ['eeg'] * n_channels # MNE uses 'eeg' as a broad type for intracranial as well in some contexts
