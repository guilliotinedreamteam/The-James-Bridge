...
def download_dataset(dataset_id: str, target_dir: str):
    ...
    logger.info(
        f"Initiating OpenNeuro AWS S3 sync for dataset: {dataset_id} into {target_path}"
    )
    ...
    try:
        ...
        logger.error(
            "AWS CLI is required to sync public datasets. Run: brew install awscli"
        )
        sys.exit(1)
    ...
    try:
        ...
        logger.info(
            f"Successfully downloaded subject 'sub-01' of dataset {dataset_id} to {target_path}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download clinical datasets from OpenNeuro S3"
    )
    parser.add_argument(
        "--dataset",
        type=str,
        required=True,
        help="OpenNeuro dataset ID (e.g., ds003620)",
    )
    parser.add_argument(
        "--target", type=str, default="datasets/openneuro_bids", help="Target directory"
    )
