...
def build_neurobridge_decoder(
    ...
    x = Conv1D(filters=cnn_filters, kernel_size=cnn_kernel_size, padding="same", activation="relu", name="conv_1")(inp)
    x = SpatialDropout1D(dropout_rate, name="sdrop_1")(x)
    x = Conv1D(filters=cnn_filters * 2, kernel_size=cnn_kernel_size, padding="same", activation="relu", name="conv_2")(x)
