...
def create_app():
    ...
    def process():
        ...
        if data is None:
            return jsonify({"error": "Malformed JSON",
                            "code": "MALFORMED_JSON"}), 400

        signal = data.get("signal")

        # Validate signal presence and type
        if signal is None:
            return jsonify({"error": "Missing 'signal' field",
                            "code": "MISSING_SIGNAL"}), 400

        if not isinstance(signal, list):
            return jsonify({"error": "'signal' must be a list",
                            "code": "INVALID_SIGNAL_TYPE"}), 400

        if len(signal) == 0:
            return jsonify({"error": "'signal' must not be empty",
                            "code": "EMPTY_SIGNAL"}), 400
        ...
        try:
            ...
            return jsonify({"error": "Non-numeric values in signal",
                            "code": "INVALID_SIGNAL_VALUES"}), 400
    ...
    def analyze():
        ...
        # Ensure at least 1-D
        if arr.ndim == 0:
            return jsonify({"error": "Scalar data not supported",
                            "code": "INVALID_DATA"}), 400
