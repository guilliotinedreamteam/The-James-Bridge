...
class HyperTuner:
    ...
    def execute_grid_search(
        self, lstm_options: list, dropout_options: list, epochs: int = 5
    ):
        """
        ...
        logger.info(
            "Starting Hyperparameter Grid Search on legitimate clinical dataset."
        )

        for units in lstm_options:
            for dropout in dropout_options:
                logger.info(
                    f"--- Tuning Run: LSTM Units={units}, Dropout={dropout} ---"
                )
        ...
        for res in results:
            logger.info(
                f"Config [Units: {res['units']}, Dropout: {res['dropout']}] -> Acc: {res['accuracy']:.4f}, Loss: {res['loss']:.4f}"
            )
