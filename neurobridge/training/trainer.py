...
class ModelTrainer:
    ...
    def __init__(self, model: 'tf.keras.Model', checkpoint_dir: str = "checkpoints"):
        if tf is None:
            raise ImportError("TensorFlow is missing. Cannot initialize training pipeline.")
    ...
    def train(self, x_train: np.ndarray, y_train: np.ndarray, x_val: np.ndarray = None, y_val: np.ndarray = None, 
              epochs: int = 50, batch_size: int = 32, model_name: str = "neurobridge_offline") -> 'tf.keras.callbacks.History':
        """
        Executes the offline training loop.
        """
        logger.info(f"Initiating training for {model_name}. Epochs: {epochs}, Batch Size: {batch_size}")
        
        validation_data = (x_val, y_val) if x_val is not None and y_val is not None else None
        if validation_data is None:
            logger.warning("No validation data provided. Model checkpointing may rely on training loss instead.")
